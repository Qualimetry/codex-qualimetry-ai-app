...
| `get_rules_based_analysis_issues_summary` | analysis-issues | Fetch issue counts by type and severity for a repo/branch (optional `pullRequest` scopes to a PR) | Qualimetry Enterprise |
| `get_rules_based_analysis_issues` | analysis-issues | Fetch paginated analysis issues with type/severity/file filtering (optional `pullRequest` for PR new code) | Qualimetry Enterprise |
