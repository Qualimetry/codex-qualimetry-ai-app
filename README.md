...
- **Surfaces review findings the moment you open a reviewed file.** Whenever Codex reads a source file in your repo, you immediately see which standards, principles, and policy violations have been raised against it, plus a standards-compliant example of how to resolve them. No command, no manual lookup. When the current branch has an open pull request, the findings are automatically scoped to the issues raised on the pull request's new code.
