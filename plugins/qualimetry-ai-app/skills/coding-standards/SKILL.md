...
- On Qualimetry Enterprise, you can also use `get_rules_based_analysis_issues`
  ...
  smells in the file being worked on, and
  `get_rules_based_analysis_security_hotspots` with a `filePath` to see the
  security-sensitive code in it that is still waiting on a human review
  decision. Hotspots are never returned by the issue tools.
