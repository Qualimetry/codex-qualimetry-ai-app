...
rules-based analysis findings for a repository branch. These are distinct from
the HITL code review issues retrieved by this skill:

- `get_rules_based_analysis_issues_summary` -- total counts by type and severity, plus the security hotspot counts
- `get_rules_based_analysis_issues` -- paginated issues with filtering
- `get_rules_based_analysis_security_hotspots` -- security-sensitive code awaiting a human review decision, which the issue tools never return
- `get_rules_based_analysis_security_hotspot` -- one hotspot with the rule's risk description and fix recommendations
